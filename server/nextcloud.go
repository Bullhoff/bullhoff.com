...
func SaveFile(w http.ResponseWriter, req *http.Request) {
	...
	if fileStatus == http.StatusNotFound {
		...
		// For merging current file on the cloud and the new file.
		// Probably have to change so that the frontend only sends changed targets, and then reads the existing file
		// Or(/and?) split the save files to one item per file
